...
{
    /// <summary>
    /// معالج استعلام جلب بيانات العقار للتحرير مع القيم الديناميكية
    ...
    public class GetPropertyForEditQueryHandler : IRequestHandler<GetPropertyForEditQuery, ResultDto<PropertyEditDto>>
    {
        ...
        public async Task<ResultDto<PropertyEditDto>> Handle(GetPropertyForEditQuery request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("بدء جلب بيانات العقار للتحرير: {PropertyId}", request.PropertyId);

                // 1. التحقق من صحة المعرفات
                if (request.PropertyId == Guid.Empty)
                    return ResultDto<PropertyEditDto>.Failure("معرف العقار غير صالح");
                ...
                if (currentUser == null || (_currentUserService.UserId != request.OwnerId && !_currentUserService.UserRoles.Contains("Admin")))
                    return ResultDto<PropertyEditDto>.Failure("ليس لديك صلاحية لتحرير هذا العقار");

                // 3. جلب العقار مع القيم الديناميكية
                ...
                if (property == null)
                    return ResultDto<PropertyEditDto>.Failure("العقار غير موجود أو لا تملك صلاحية الوصول إليه");
                ...
                return ResultDto<PropertyEditDto>.Ok(dto, "تم جلب بيانات العقار للتحرير بنجاح");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "خطأ في جلب بيانات العقار للتحرير: {PropertyId}", request.PropertyId);
