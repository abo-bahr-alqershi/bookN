...
{
    /// <summary>
    /// معالج استعلام جلب تفاصيل العقار مع الحقول الديناميكية والوحدات الاختيارية
    ...
    public class GetPropertyDetailsQueryHandler : IRequestHandler<GetPropertyDetailsQuery, ResultDto<PropertyDetailsDto>>
    {
        ...
        public async Task<ResultDto<PropertyDetailsDto>> Handle(GetPropertyDetailsQuery request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("بدء جلب تفاصيل العقار: {PropertyId}", request.PropertyId);
                ...
                return ResultDto<PropertyDetailsDto>.Ok(dto, "تم استرجاع تفاصيل العقار بنجاح");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "خطأ في جلب تفاصيل العقار: {PropertyId}", request.PropertyId);
