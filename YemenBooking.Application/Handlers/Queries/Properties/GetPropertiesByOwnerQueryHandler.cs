...
{
    /// <summary>
    /// معالج استعلام الحصول على عقارات المالك
    ...
    public class GetPropertiesByOwnerQueryHandler : IRequestHandler<GetPropertiesByOwnerQuery, PaginatedResult<PropertyDto>>
    {
        ...
        {
            _logger.LogInformation("بدء جلب عقارات المالك: {OwnerId} - الصفحة: {PageNumber}, الحجم: {PageSize}",
            ...
            if (currentUser == null)
            {
                ...
                throw new UnauthorizedAccessException("يجب تسجيل الدخول لعرض عقارات المالك");
            }
            var role = _currentUserService.Role;
            if (role != "Admin" && _currentUserService.UserId != request.OwnerId)
            {
                _logger.LogWarning("ليس لدى المستخدم الصلاحيات اللازمة لعرض عقارات هذا المالك");
                throw new UnauthorizedAccessException("ليس لديك صلاحية لعرض عقارات هذا المالك");
            }
            ...
            _logger.LogInformation("تم جلب {Count} عقار من إجمالي {TotalCount} لعقارات المالك", dtos.Count, totalCount);
