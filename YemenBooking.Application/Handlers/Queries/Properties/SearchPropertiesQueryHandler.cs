...
{
    /// <summary>
    /// معالج استعلام البحث عن العقارات المتقدم
    ...
    public class SearchPropertiesQueryHandler : IRequestHandler<SearchPropertiesQuery, PaginatedResult<PropertyDto>>
    {
        ...
        {
            _logger.LogInformation("بدء البحث عن العقارات - موقع: {Location}, السعر من {MinPrice} إلى {MaxPrice}, الصفحة: {PageNumber}, الحجم: {PageSize}",
            ...
            {
                "Admin" => query,
                "PropertyOwner" => query.Where(p => p.IsApproved || p.OwnerId == userId),
            ...
            _logger.LogInformation("تم جلب {Count} عقار من إجمالي {TotalCount} بعد البحث المتقدم", dtos.Count, totalCount);
