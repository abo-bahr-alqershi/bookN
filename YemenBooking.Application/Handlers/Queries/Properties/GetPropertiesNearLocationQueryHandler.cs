...
{
    /// <summary>
    /// معالج استعلام الحصول على العقارات القريبة من موقع معين
    ...
    public class GetPropertiesNearLocationQueryHandler : IRequestHandler<GetPropertiesNearLocationQuery, PaginatedResult<PropertyDto>>
    {
        ...
        {
            _logger.LogInformation("بدء جلب العقارات القريبة من ({Latitude}, {Longitude}) ضمن مسافة {Distance} كم - الصفحة: {PageNumber}, الحجم: {PageSize}",
                request.Latitude, request.Longitude, request.Distance, request.PageNumber, request.PageSize);

            // جلب جميع العقارات غير المحذوفة
            ...
            {
                "Admin" => allProperties,
                "PropertyOwner" => allProperties.Where(p => p.IsApproved || p.OwnerId == userId).ToList(),
            ...
            _logger.LogInformation("تم جلب {Count} عقار من إجمالي {TotalCount} القريبة", dtos.Count, totalCount);
