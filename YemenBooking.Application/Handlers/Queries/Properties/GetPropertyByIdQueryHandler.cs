...
{
    /// <summary>
    /// معالج استعلام الحصول على بيانات العقار بواسطة المعرف
    ...
    public class GetPropertyByIdQueryHandler : IRequestHandler<GetPropertyByIdQuery, ResultDto<PropertyDetailsDto>>
    {
        ...
        public async Task<ResultDto<PropertyDetailsDto>> Handle(GetPropertyByIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("بدء معالجة استعلام بيانات العقار: {PropertyId}", request.PropertyId);

                // 1. التحقق من صحة المعرف
                if (request.PropertyId == Guid.Empty)
                    return ResultDto<PropertyDetailsDto>.Failure("معرف العقار غير صالح");

                // 2. جلب العقار مع البيانات المرتبطة
                ...
                if (property == null)
                    return ResultDto<PropertyDetailsDto>.Failure($"العقار بالمعرف {request.PropertyId} غير موجود");
                ...
                {
                    if (currentUser == null)
                        return ResultDto<PropertyDetailsDto>.Failure("يجب تسجيل الدخول لعرض هذا العقار");

                    var roles = _currentUserService.UserRoles;
                    if (property.OwnerId != _currentUserService.UserId && !roles.Contains("Admin"))
                        return ResultDto<PropertyDetailsDto>.Failure("ليس لديك صلاحية لعرض هذا العقار");
                }
                ...
                return ResultDto<PropertyDetailsDto>.Ok(dto, "تم استرجاع بيانات العقار بنجاح");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "خطأ في معالجة استعلام بيانات العقار: {PropertyId}", request.PropertyId);
