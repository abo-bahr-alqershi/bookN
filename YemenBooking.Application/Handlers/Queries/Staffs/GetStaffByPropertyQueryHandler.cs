...
{
    /// <summary>
    /// معالج استعلام الحصول على موظفي العقار
    ...
    public class GetStaffByPropertyQueryHandler : IRequestHandler<GetStaffByPropertyQuery, PaginatedResult<StaffDto>>
    {
        ...
        {
            _logger.LogInformation("بدء جلب موظفي العقار: {PropertyId}, Page={Page}, Size={Size}", request.PropertyId, request.PageNumber, request.PageSize);
            ...
            {
                _logger.LogWarning("ليس لدى المستخدم صلاحية الوصول إلى موظفي هذا العقار");
                throw new ForbiddenException("ليس لديك صلاحية الوصول إلى موظفي هذا العقار");
            }
