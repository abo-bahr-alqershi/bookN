...
{
    /// <summary>
    /// معالج استعلام مؤشرات أداء العقار
    ...
    public class GetPropertyPerformanceQueryHandler : IRequestHandler<GetPropertyPerformanceQuery, ResultDto<PropertyPerformanceDto>>
    {
        ...
        {
            _logger.LogInformation("بدء جلب أداء العقار: {PropertyId} من {StartDate} إلى {EndDate}",
                request.PropertyId, request.StartDate, request.EndDate);

            // Validate inputs
            if (request.PropertyId == Guid.Empty)
            {
                _logger.LogWarning("معرف العقار غير صالح");
                return ResultDto<PropertyPerformanceDto>.Failure("معرف العقار غير صالح");
            }
            ...
            if (userRole != "Admin" && _currentUserService.PropertyId != request.PropertyId)
            {
                ...
                return ResultDto<PropertyPerformanceDto>.Failure("ليس لديك صلاحية لعرض تقرير أداء هذا العقار");
            }
            ...
            if (metrics == null)
            {
                _logger.LogError("فشل في تحليل بيانات مؤشرات الأداء للعقار: {PropertyId}", request.PropertyId);
