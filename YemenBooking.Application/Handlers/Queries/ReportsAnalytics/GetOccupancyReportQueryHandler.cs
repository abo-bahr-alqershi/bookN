...
namespace YemenBooking.Application.Handlers.Queries.ReportsAnalytics
{
    ...
    public class GetOccupancyReportQueryHandler : IRequestHandler<GetOccupancyReportQuery, ResultDto<OccupancyReportDto>>
    {
        ...
        {
            _logger.LogInformation("بدء جلب تقرير الإشغال للعقار: {PropertyId} من {StartDate} إلى {EndDate}",
            ...
            if (request.PropertyId == Guid.Empty)
            {
                ...
                return ResultDto<OccupancyReportDto>.Failure("معرف العقار غير صالح");
            }
            ...
            if (role != "Admin" && _currentUserService.PropertyId != request.PropertyId)
            {
                ...
                return ResultDto<OccupancyReportDto>.Failure("ليس لديك صلاحية لعرض تقرير الإشغال لهذا العقار");
            }
