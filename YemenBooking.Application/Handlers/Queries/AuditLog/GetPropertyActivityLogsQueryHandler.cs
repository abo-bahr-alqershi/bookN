...
{
    /// <summary>
    /// معالج استعلام لجلب آخر سجلات النشاطات للمالكين وموظفي العقار
    ...
    public class GetPropertyActivityLogsQueryHandler : IRequestHandler<GetPropertyActivityLogsQuery, PaginatedResult<AuditLogDto>>
    {
        ...
        {
            _logger.LogInformation("بدء جلب سجلات النشاطات للمالكين وموظفي العقار: {PropertyId}", request.PropertyId);

            if (!request.PropertyId.HasValue || request.PropertyId == Guid.Empty)
                throw new BusinessRuleException("BadRequest", "معرف العقار غير صالح");
            ...
            {
                throw new BusinessRuleException("Forbidden", "ليس لديك صلاحية لعرض سجلات النشاطات لهذا العقار");
            }

            var property = await _propertyRepository.GetPropertyByIdAsync(propertyId, cancellationToken);
            if (property == null)
                throw new BusinessRuleException("NotFound", "العقار غير موجود");
