...
{
    /// <summary>
    /// معالج استعلام تحليل نافذة الحجز لعقار محدد
    ...
    public class GetBookingWindowAnalysisQueryHandler : IRequestHandler<GetBookingWindowAnalysisQuery, ResultDto<BookingWindowDto>>
    {
        ...
        {
            _logger.LogInformation("بدء تحليل نافذة الحجز للعقار: {PropertyId}", request.PropertyId);

            if (request.PropertyId == Guid.Empty)
            {
                _logger.LogWarning("معرف العقار غير صالح");
                return ResultDto<BookingWindowDto>.Failure("معرف العقار غير صالح");
            }
