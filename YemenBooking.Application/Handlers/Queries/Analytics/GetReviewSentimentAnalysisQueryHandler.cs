...
{
    /// <summary>
    /// معالج استعلام تحليل مشاعر التقييمات لعقار محدد
    ...
    public class GetReviewSentimentAnalysisQueryHandler : IRequestHandler<GetReviewSentimentAnalysisQuery, ResultDto<ReviewSentimentDto>>
    {
        ...
        {
            _logger.LogInformation("بدء تحليل مشاعر التقييمات للعقار: {PropertyId}", request.PropertyId);

            if (request.PropertyId == Guid.Empty)
            {
                _logger.LogWarning("معرف العقار غير صالح");
                return ResultDto<ReviewSentimentDto>.Failure("معرف العقار غير صالح");
            }
