...
{
    /// <summary>
    /// معالج استعلام الحصول على حجوزات عقار معين
    ...
    public class GetBookingsByPropertyQueryHandler : IRequestHandler<GetBookingsByPropertyQuery, PaginatedResult<BookingDto>>
    {
        ...
        {
            _logger.LogInformation("جاري معالجة استعلام حجوزات العقار: {PropertyId}, الصفحة: {PageNumber}, الحجم: {PageSize}", request.PropertyId, request.PageNumber, request.PageSize);
            ...
            if (!roles.Contains("Admin") && _currentUserService.PropertyId != request.PropertyId)
                throw new UnauthorizedAccessException("ليس لديك صلاحية لعرض حجوزات هذا العقار");
            ...
            _logger.LogInformation("تم جلب {Count} حجز من إجمالي {TotalCount} لحجوزات العقار: {PropertyId}", dtos.Count, totalCount, request.PropertyId);
