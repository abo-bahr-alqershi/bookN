...
{
    /// <summary>
    /// معالج أمر تحديث بيانات خدمة العقار
    /// </summary>
    public class UpdatePropertyServiceCommandHandler : IRequestHandler<UpdatePropertyServiceCommand, ResultDto<bool>>
    {
        ...
        {
            _logger.LogInformation("بدء تحديث خدمة العقار: ServiceId={ServiceId}", request.ServiceId);
            ...
            if (property == null)
                return ResultDto<bool>.Failed("العقار المرتبط بالخدمة غير موجود");
            ...
            if (!string.IsNullOrWhiteSpace(request.Name) && !string.Equals(service.Name, request.Name.Trim(), StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (duplicate)
                    return ResultDto<bool>.Failed("يوجد خدمة أخرى بنفس الاسم لهذا العقار");
