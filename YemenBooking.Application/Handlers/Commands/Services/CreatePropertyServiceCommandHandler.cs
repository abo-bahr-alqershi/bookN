...
{
    /// <summary>
    /// معالج أمر إنشاء خدمة جديدة للعقار
    /// </summary>
    public class CreatePropertyServiceCommandHandler : IRequestHandler<CreatePropertyServiceCommand, ResultDto<Guid>>
    {
        ...
        {
            _logger.LogInformation("بدء إنشاء خدمة للعقار: PropertyId={PropertyId}, Name={Name}", request.PropertyId, request.Name);
            ...
            if (_currentUserService.Role != "Admin" && property.OwnerId != _currentUserService.UserId)
                return ResultDto<Guid>.Failed("غير مصرح لك بإنشاء خدمة لهذا العقار");
            ...
            if (exists)
                return ResultDto<Guid>.Failed("يوجد خدمة بنفس الاسم لهذا العقار");
            ...
            await _auditService.LogBusinessOperationAsync(
                ...
                $"تم إنشاء خدمة جديدة {created.Id} للعقار {created.PropertyId}",
                created.Id,
                "PropertyService",
