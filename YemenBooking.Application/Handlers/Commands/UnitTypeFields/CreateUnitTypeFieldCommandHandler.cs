...
public class CreateUnitTypeFieldCommandHandler : IRequestHandler<CreateUnitTypeFieldCommand, ResultDto<string>>
{
    ...
    public async Task<ResultDto<string>> Handle(CreateUnitTypeFieldCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var unitTypeField = new UnitTypeField
            {
                ...
                FieldTypeId = Guid.Parse(request.FieldTypeId),
    ...
    private async Task<ResultDto<string>> ValidateInputAsync(CreateUnitTypeFieldCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!Guid.TryParse(request.FieldTypeId, out _))
        {
    ...
    private async Task<ResultDto<string>> ValidateEntitiesExistenceAsync(CreateUnitTypeFieldCommand request, CancellationToken cancellationToken)
    {
        ...
        var fieldTypeId = Guid.Parse(request.FieldTypeId);
        var fieldType = await _fieldTypeRepository.GetFieldTypeByIdAsync(fieldTypeId, cancellationToken);
        if (fieldType == null)
        {
            _logger.LogWarning("نوع الحقل غير موجود: {FieldTypeId}", request.FieldTypeId);
            return ResultDto<string>.Failure("نوع الحقل غير موجود");
        }

        // التحقق من أن نوع الحقل نشط
        if (!fieldType.IsActive)
        {
            _logger.LogWarning("نوع الحقل غير نشط: {FieldTypeId}", request.FieldTypeId);
            return ResultDto<string>.Failure("نوع الحقل غير نشط");
        }
    ...
    private async Task<ResultDto<string>> ValidateBusinessRulesAsync(CreateUnitTypeFieldCommand request, CancellationToken cancellationToken)
    {
        ...
        if (fieldType?.Name != null && (fieldType.Name.ToLower() == "select" || fieldType.Name.ToLower() == "multi_select"))
        {
            if (request.FieldOptions == null || !request.FieldOptions.Any())
            {
                return ResultDto<string>.Failure("حقول الاختيار تتطلب خيارات");
            }
        }
