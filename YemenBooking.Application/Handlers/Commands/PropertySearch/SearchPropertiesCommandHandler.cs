...
public class SearchPropertiesCommandHandler : IRequestHandler<SearchPropertiesCommand, ResultDto<PropertySearchResultDto>>
{
    ...
    public async Task<ResultDto<PropertySearchResultDto>> Handle(SearchPropertiesCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            _logger.LogInformation("بدء معالجة أمر البحث في العقارات");
        ...
        {
            stopwatch.Stop();
            _logger.LogError(ex, "خطأ أثناء البحث في العقارات");
            return ResultDto<PropertySearchResultDto>.Failure("حدث خطأ أثناء البحث في العقارات");
        }
    }
        ...
        SearchPropertiesCommand request, CancellationToken cancellationToken)
    {
        return Task.FromResult(new PropertySearchParameters
        {
            ...
            IsApproved = true, // نعرض فقط العقارات المعتمدة
            PageNumber = request.PageNumber,
            PageSize = request.PageSize
        });
    }
    ...
    /// <summary>
    /// تحويل العقارات إلى عناصر البحث
    ...
    private async Task LogSearchOperationAsync(SearchPropertiesCommand request, int resultCount, long durationMs)
    {
        try
        {
            await _auditService.LogAsync(
                ...
                $"بحث في العقارات - النتائج: {resultCount}, المدة: {durationMs}ms",
