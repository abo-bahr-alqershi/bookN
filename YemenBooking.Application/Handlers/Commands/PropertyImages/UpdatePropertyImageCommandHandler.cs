...
/// <summary>
/// معالج أمر تحديث صورة العقار
/// Property image update command handler
/// 
/// يعالج تحديث صورة عقار موجودة ويتضمن:
...
public class UpdatePropertyImageCommandHandler : IRequestHandler<UpdatePropertyImageCommand, ResultDto<bool>>
{
    ...
    public async Task<ResultDto<bool>> Handle(UpdatePropertyImageCommand request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("بدء معالجة تحديث صورة العقار - ImageId: {ImageId}", request.ImageId);
            ...
            var result = await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                ...
                await _auditService.LogActivityAsync(
                    ...
                    $"تم تحديث صورة العقار - ID: {updatedImage.Id}",
                    "PropertyImage",
                ...
                _logger.LogInformation("تم تحديث صورة العقار بنجاح - ImageId: {ImageId}", updatedImage.Id);
        ...
        {
            _logger.LogError(ex, "خطأ أثناء تحديث صورة العقار - ImageId: {ImageId}", request.ImageId);
            return ResultDto<bool>.Failure("فشل في تحديث صورة العقار");
        }
    }
    ...
    private async Task<ResultDto<bool>> ValidatePermissionsAsync(PropertyImage image, CancellationToken cancellationToken)
    {
        ...
        // التحقق من الصلاحيات حسب العقار أو الوحدة
    ...
    private async Task SendNotificationsAsync(PropertyImage image, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (image.PropertyId.HasValue)
            {
                ...
                if (property != null && property.OwnerId != _currentUserService.UserId)
                {
                    var notificationRequest = new NotificationRequest
                    {
                        ...
                        Title = "تم تحديث صورة العقار",
                        Message = $"تم تحديث صورة في عقارك: {property.Name}",
            ...
            // إشعار مالك الوحدة (إذا كانت مختلفة عن مالك العقار)
