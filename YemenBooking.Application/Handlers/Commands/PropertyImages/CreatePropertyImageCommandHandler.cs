...
/// <summary>
/// معالج أمر إنشاء صورة عقار جديدة
/// Create property image command handler
/// 
/// يقوم بإنشاء صورة جديدة لعقار أو وحدة ويشمل:
/// - التحقق من صحة البيانات المدخلة
/// - التحقق من وجود العقار أو الوحدة
...
public class CreatePropertyImageCommandHandler : IRequestHandler<CreatePropertyImageCommand, ResultDto<Guid>>
{
    ...
    public async Task<ResultDto<Guid>> Handle(CreatePropertyImageCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            _logger.LogInformation("بدء معالجة أمر إنشاء صورة عقار جديدة - PropertyId: {PropertyId}, UnitId: {UnitId}", 
            ...
            {
                _logger.LogWarning("العقار أو الوحدة غير موجودة - PropertyId: {PropertyId}, UnitId: {UnitId}", 
                    request.PropertyId, request.UnitId);
            ...
            var result = await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                ...
                await _auditService.LogActivityAsync(
                    ...
                    $"تم إنشاء صورة عقار جديدة - ID: {createdImage.Id}",
                    "PropertyImage",
                ...
                _logger.LogInformation("تم إنشاء صورة العقار بنجاح - ImageId: {ImageId}", createdImage.Id);

                return createdImage.Id;
            }, cancellationToken);

            return ResultDto<Guid>.Ok(result, "تم إنشاء صورة العقار بنجاح");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "خطأ غير متوقع أثناء معالجة أمر إنشاء صورة العقار");
            return ResultDto<Guid>.Failure("حدث خطأ غير متوقع أثناء إنشاء صورة العقار");
        }
    }
    #endregion
    ...
    private async Task<ResultDto<bool>> ValidateRequestAsync(CreatePropertyImageCommand request, CancellationToken cancellationToken)
    {
        ...
        // يجب تحديد عقار أو وحدة واحدة على الأقل
        // Must specify either property or unit
        if (!request.PropertyId.HasValue && !request.UnitId.HasValue)
        {
            errors.Add("يجب تحديد معرف العقار أو معرف الوحدة");
        }

        // لا يمكن تحديد عقار ووحدة في نفس الوقت
        // Cannot specify both property and unit
        if (request.PropertyId.HasValue && request.UnitId.HasValue)
        {
            errors.Add("لا يمكن تعيين الصورة لعقار ووحدة في نفس الوقت");
        }
    ...
    private async Task<ResultDto<bool>> CheckUserAuthorizationAsync(CreatePropertyImageCommand request, CancellationToken cancellationToken)
    {
        ...
        // التحقق من صلاحية إنشاء صور العقارات
        // Check property image creation permission
        var hasPermission = _currentUserService.Permissions.Contains("PropertyImages.Create");
        if (!hasPermission)
        {
            return ResultDto<bool>.Failure("المستخدم لا يملك صلاحية إنشاء صور العقارات");
        }

        // إذا كان العقار محدد، التحقق من ملكية العقار أو الصلاحية لإدارته
        ...
        if (request.PropertyId.HasValue)
        {
            ...
            {
                return ResultDto<bool>.Failure("المستخدم لا يملك صلاحية إدارة هذا العقار");
            }
        }
    ...
    private async Task<ResultDto<bool>> VerifyPropertyOrUnitExistsAsync(CreatePropertyImageCommand request, CancellationToken cancellationToken)
    {
        if (request.PropertyId.HasValue)
        {
            ...
            if (property == null)
            {
                return ResultDto<bool>.Failure("العقار المحدد غير موجود");
            }

            // التحقق من أن العقار ليس محذوف
            // Check property is not deleted
            if (property.IsDeleted)
            {
                return ResultDto<bool>.Failure("لا يمكن إضافة صور لعقار محذوف");
            }
        }
    ...
    private PropertyImage CreatePropertyImageEntity(CreatePropertyImageCommand request, ProcessedImageResult? processedImage)
    {
        ...
        return new PropertyImage
        {
            ...
            Name = !string.IsNullOrEmpty(request.Caption) ? request.Caption : "صورة عقار",
    ...
    private async Task SendNotificationsAsync(PropertyImage image, CancellationToken cancellationToken)
    {
            ...
            if (image.PropertyId.HasValue)
            {
                ...
                if (property != null && property.OwnerId != _currentUserService.UserId)
                {
                    ...
                    var notificationRequest = new NotificationRequest
                    {
                        ...
                        Message = $"تم إضافة صورة جديدة للعقار: {property.Name}",
        ...
        {
            _logger.LogWarning(ex, "فشل في إرسال الإشعارات لصورة العقار: {ImageId}", image.Id);
