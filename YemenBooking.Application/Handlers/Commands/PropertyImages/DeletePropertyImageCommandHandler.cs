...
public class DeletePropertyImageCommandHandler : IRequestHandler<DeletePropertyImageCommand, ResultDto<bool>>
{
    ...
    public async Task<ResultDto<bool>> Handle(DeletePropertyImageCommand request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("بدء معالجة حذف صورة العقار - ImageId: {ImageId}", request.ImageId);
            ...
            var result = await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                ...
                await _auditService.LogActivityAsync(
                    ...
                    $"تم حذف صورة العقار - ID: {deletedImage.Id}",
                    "PropertyImage",
                ...
                _logger.LogInformation("تم حذف صورة العقار بنجاح - ImageId: {ImageId}", deletedImage.Id);
        ...
        {
            _logger.LogError(ex, "خطأ أثناء حذف صورة العقار - ImageId: {ImageId}", request.ImageId);
            return ResultDto<bool>.Failure("فشل في حذف صورة العقار");
        }
    }
    #endregion
            ...
            if (deletedImage.PropertyId.HasValue)
            {
                ...
                if (property != null && property.OwnerId != _currentUserService.UserId)
                {
                    var notificationRequest = new NotificationRequest
                    {
                        ...
                        Title = "تم حذف صورة العقار",
                        Message = $"تم حذف صورة من عقارك: {property.Name}",
