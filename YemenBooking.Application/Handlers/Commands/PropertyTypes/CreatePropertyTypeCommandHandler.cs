...
namespace YemenBooking.Application.Handlers.Commands.PropertyTypes
{
    ...
    public class CreatePropertyTypeCommandHandler : IRequestHandler<CreatePropertyTypeCommand, ResultDto<Guid>>
    {
        ...
        {
            _logger.LogInformation("بدء إنشاء نوع عقار جديد: Name={Name}", request.Name);

            // التحقق من المدخلات
            if (string.IsNullOrWhiteSpace(request.Name))
                return ResultDto<Guid>.Failed("اسم نوع العقار مطلوب");

            // التحقق من الصلاحيات (مسؤول)
            if (_currentUserService.Role != "Admin")
                return ResultDto<Guid>.Failed("غير مصرح لك بإنشاء نوع عقار");

            // التحقق من التكرار
            bool exists = await _repository.ExistsAsync(pt => pt.Name.Trim() == request.Name.Trim(), cancellationToken);
            if (exists)
                return ResultDto<Guid>.Failed("يوجد نوع عقار بنفس الاسم");
            ...
            await _auditService.LogBusinessOperationAsync(
                "CreatePropertyType",
                $"تم إنشاء نوع عقار جديد {created.Id} باسم {created.Name}",
                created.Id,
                "PropertyType",
            ...
            _logger.LogInformation("اكتمل إنشاء نوع العقار: PropertyTypeId={PropertyTypeId}", created.Id);
            return ResultDto<Guid>.Succeeded(created.Id, "تم إنشاء نوع العقار بنجاح");
        }
    }
} 