...
namespace YemenBooking.Application.Handlers.Commands.PropertyTypes
{
    ...
    public class UpdatePropertyTypeCommandHandler : IRequestHandler<UpdatePropertyTypeCommand, ResultDto<bool>>
    {
        ...
        {
            _logger.LogInformation("بدء تحديث نوع العقار: Id={PropertyTypeId}", request.PropertyTypeId);
            ...
            if (_currentUserService.Role != "Admin")
                return ResultDto<bool>.Failed("غير مصرح لك بتحديث نوع العقار");
            ...
            if (!string.Equals(type.Name, request.Name?.Trim(), StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (duplicate)
                    return ResultDto<bool>.Failed("يوجد نوع عقار آخر بنفس الاسم");
            ...
            await _auditService.LogBusinessOperationAsync(
                "UpdatePropertyType",
                $"تم تحديث نوع العقار {request.PropertyTypeId}",
            ...
            _logger.LogInformation("اكتمل تحديث نوع العقار: Id={PropertyTypeId}", request.PropertyTypeId);
            return ResultDto<bool>.Succeeded(true, "تم تحديث نوع العقار بنجاح");
        }
    }
} 