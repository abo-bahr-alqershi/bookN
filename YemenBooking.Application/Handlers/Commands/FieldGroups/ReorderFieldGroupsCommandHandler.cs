...
namespace YemenBooking.Application.Handlers.Commands.FieldGroups
{
    ...
    public class ReorderFieldGroupsCommandHandler : IRequestHandler<ReorderFieldGroupsCommand, ResultDto<bool>>
    {
        ...
        {
            _logger.LogInformation("بدء معالجة أمر إعادة ترتيب مجموعات الحقول لنوع العقار: {PropertyTypeId}", request.PropertyTypeId);

            // التحقق من صحة معرف نوع العقار
            if (!Guid.TryParse(request.PropertyTypeId, out var propertyTypeId))
                throw new BusinessRuleException("InvalidPropertyTypeId", "معرف نوع العقار غير صالح");
            ...
            await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                foreach (var order in request.GroupOrders)
                {
                    ...
                    if (group.UnitTypeId != propertyTypeId)
                        throw new BusinessRuleException("GroupOwnershipMismatch", "المجموعة لا تنتمي لنوع العقار المحدد");
