...
namespace YemenBooking.Application.Handlers.Commands.FieldGroups
{
    ...
    public class DeleteFieldGroupCommandHandler : IRequestHandler<DeleteFieldGroupCommand, ResultDto<bool>>
    {
        ...
        public async Task<ResultDto<bool>> Handle(DeleteFieldGroupCommand request, CancellationToken cancellationToken)
        {
            ...
            var siblings = await _fieldGroupRepository.GetGroupsByPropertyTypeIdAsync(existingGroup.UnitTypeId, cancellationToken);
            if (siblings != null && siblings.Count() <= 1)
                throw new ValidationException("لا يمكن حذف المجموعة الوحيدة لنوع العقار");
