...
public class CreateFieldGroupCommandHandler : IRequestHandler<CreateFieldGroupCommand, ResultDto<string>>
{
    ...
    public async Task<ResultDto<string>> Handle(CreateFieldGroupCommand request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("بدء معالجة أمر إنشاء مجموعة الحقول لنوع العقار: {PropertyTypeId}", request.PropertyTypeId);
            ...
            // الخطوة 2: التحقق من وجود نوع العقار
            var propertyTypeId = Guid.Parse(request.PropertyTypeId);
            var propertyType = await _propertyTypeRepository.GetByIdAsync(propertyTypeId, cancellationToken);
            if (propertyType == null)
            {
                _logger.LogWarning("نوع العقار غير موجود: {PropertyTypeId}", request.PropertyTypeId);
                return ResultDto<string>.Failure("نوع العقار غير موجود");
            }
            ...
            var businessRulesValidation = await ValidateBusinessRulesAsync(request, propertyTypeId, cancellationToken);
            ...
            await ReorderExistingGroupsIfNeededAsync(propertyTypeId, request.SortOrder, cancellationToken);
    ...
    private async Task<ResultDto<string>> ValidateInputAsync(CreateFieldGroupCommand request, CancellationToken cancellationToken)
    {
        ...
        // التحقق من صحة معرف نوع العقار
        if (!Guid.TryParse(request.PropertyTypeId, out _))
        {
            _logger.LogWarning("معرف نوع العقار غير صحيح: {PropertyTypeId}", request.PropertyTypeId);
            return ResultDto<string>.Failure("معرف نوع العقار غير صحيح");
        }
    ...
    {
        // التحقق من عدم وجود مجموعة بنفس الاسم ضمن نفس نوع العقار
        var existingGroups = await _fieldGroupRepository.GetGroupsByPropertyTypeIdAsync(propertyTypeId, cancellationToken);
        if (existingGroups.Any(g => g.GroupName.Equals(request.GroupName, StringComparison.OrdinalIgnoreCase)))
        {
            _logger.LogWarning("يوجد مجموعة حقول بنفس الاسم في نوع العقار: {GroupName}", request.GroupName);
            return ResultDto<string>.Failure($"يوجد مجموعة حقول بالاسم '{request.GroupName}' مسبقاً في هذا النوع من العقارات");
        }
