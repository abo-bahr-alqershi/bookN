...
namespace YemenBooking.Application.Handlers.Commands.Properties
{
    ...
    public class DeletePropertyCommandHandler : IRequestHandler<DeletePropertyCommand, ResultDto<bool>>
    {
        ...
        {
            _logger.LogInformation("بدء حذف العقار: PropertyId={PropertyId}", request.PropertyId);
            ...
            if (_currentUserService.Role != "Admin" && property.OwnerId != _currentUserService.UserId)
                return ResultDto<bool>.Failed("غير مصرح لك بحذف هذا العقار");
            ...
            if (hasActiveBookings)
                return ResultDto<bool>.Failed("لا يمكن حذف العقار لوجود حجوزات نشطة أو مستقبلية");

            var success = await _propertyRepository.DeletePropertyAsync(request.PropertyId, cancellationToken);
            if (!success)
                return ResultDto<bool>.Failed("فشل حذف العقار");

            // تسجيل العملية في سجل التدقيق
            await _auditService.LogBusinessOperationAsync(
                "DeleteProperty",
                $"تم حذف العقار {request.PropertyId}",
                request.PropertyId,
                "Property",
            ...
            _logger.LogInformation("اكتمل حذف العقار: PropertyId={PropertyId}", request.PropertyId);
            return ResultDto<bool>.Succeeded(true, "تم حذف العقار بنجاح");
        }
    }
} 