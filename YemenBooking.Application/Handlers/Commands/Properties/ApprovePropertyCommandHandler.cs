...
namespace YemenBooking.Application.Handlers.Commands.Properties
{
    ...
    public class ApprovePropertyCommandHandler : IRequestHandler<ApprovePropertyCommand, ResultDto<bool>>
    {
        ...
        {
            _logger.LogInformation("بدء الموافقة على العقار: PropertyId={PropertyId}", request.PropertyId);
            ...
            if (_currentUserService.Role != "Admin")
                return ResultDto<bool>.Failed("غير مصرح لك بالموافقة على العقار");
            ...
            if (property.IsApproved)
                return ResultDto<bool>.Failed("العقار معتمد مسبقاً");

            // تنفيذ الموافقة
            var success = await _propertyRepository.ApprovePropertyAsync(request.PropertyId, cancellationToken);
            if (!success)
                return ResultDto<bool>.Failed("فشل الموافقة على العقار");

            // تسجيل العملية في سجل التدقيق
            await _auditService.LogBusinessOperationAsync(
                "ApproveProperty",
                $"تمت الموافقة على العقار {request.PropertyId}",
                request.PropertyId,
                "Property",
            ...
            await _notificationService.SendAsync(new NotificationRequest
            {
                ...
                Message = $"تمت الموافقة على عقارك '{property.Name}' بنجاح"
            }, cancellationToken);

            _logger.LogInformation("اكتملت الموافقة على العقار: PropertyId={PropertyId}", request.PropertyId);
            return ResultDto<bool>.Succeeded(true, "تمت الموافقة على العقار بنجاح");
        }
    }
} 