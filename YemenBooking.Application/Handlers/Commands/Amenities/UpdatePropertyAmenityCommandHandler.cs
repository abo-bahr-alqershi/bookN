...
{
    /// <summary>
    /// معالج أمر تحديث إعدادات المرفق في العقار
    /// </summary>
    public class UpdatePropertyAmenityCommandHandler : IRequestHandler<UpdatePropertyAmenityCommand, ResultDto<bool>>
    {
        ...
        /// <summary>
        /// تنفيذ منطق تحديث إعدادات المرفق في العقار
        /// </summary>
        public async Task<ResultDto<bool>> Handle(UpdatePropertyAmenityCommand request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("بدء معالجة أمر تحديث إعدادات المرفق في العقار: PropertyId={PropertyId}, AmenityId={AmenityId}", request.PropertyId, request.AmenityId);
            ...
            try
            {
                ...
                // العثور على تخصيص نوع العقار والمرفق
                ...
                // العثور على علاقة المرفق بالعقار
                ...
                if (propertyAmenity == null)
                    return ResultDto<bool>.Failed("المرفق غير مرتبط بهذا العقار");

                // التحقق من الصلاحيات
                if (_currentUserService.Role != "Admin" && property.OwnerId != _currentUserService.UserId)
                    return ResultDto<bool>.Failed("ليس لديك صلاحية لتحديث إعدادات المرفق في هذا العقار");
                ...
                await _auditService.LogActivityAsync(
                    ...
                    "UPDATE",
                    "تم تحديث إعدادات المرفق في العقار",
                    oldValues,
                    propertyAmenity,
                    cancellationToken);

                _logger.LogInformation("تم تحديث إعدادات المرفق في العقار: PropertyAmenityId={PaId}", propertyAmenity.Id);
                return ResultDto<bool>.Succeeded(true, "تم تحديث إعدادات المرفق في العقار بنجاح");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "خطأ في معالجة أمر تحديث إعدادات المرفق في العقار: PropertyId={PropertyId}, AmenityId={AmenityId}", request.PropertyId, request.AmenityId);
                return ResultDto<bool>.Failed("حدث خطأ أثناء تحديث إعدادات المرفق في العقار");
            }
        }
    }
} 