...
{
    /// <summary>
    /// معالج أمر تخصيص المرفق لنوع العقار
    /// </summary>
    public class AssignAmenityToPropertyTypeCommandHandler : IRequestHandler<AssignAmenityToPropertyTypeCommand, ResultDto<bool>>
    {
        ...
        /// <summary>
        /// تنفيذ منطق تخصيص المرفق لنوع العقار
        /// </summary>
        public async Task<ResultDto<bool>> Handle(AssignAmenityToPropertyTypeCommand request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("بدء معالجة أمر تخصيص المرفق لنوع العقار: PropertyTypeId={PropertyTypeId}, AmenityId={AmenityId}", request.PropertyTypeId, request.AmenityId);
            ...
            try
            {
                ...
                if (request.PropertyTypeId == Guid.Empty)
                    errors.Add("معرف نوع العقار مطلوب");
                ...
                // التحقق من وجود نوع العقار والمرفق
                ...
                if (_currentUserService.Role != "Admin")
                    return ResultDto<bool>.Failed("غير مصرح لك بتخصيص المرفق لنوع العقار");
                ...
                if (exists)
                    return ResultDto<bool>.Failed("المرفق مخصص مسبقاً لنوع العقار");
                ...
                await _auditService.LogActivityAsync(
                    ...
                    pta.Id.ToString(),
                    "CREATE",
                    "تم تخصيص المرفق لنوع العقار بنجاح",
                    null,
                    pta,
                    cancellationToken);

                _logger.LogInformation("تم تخصيص المرفق لنوع العقار: PtaId={PtaId}", pta.Id);
                return ResultDto<bool>.Succeeded(true, "تم تخصيص المرفق لنوع العقار بنجاح");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "خطأ في معالجة أمر تخصيص المرفق لنوع العقار: PropertyTypeId={PropertyTypeId}, AmenityId={AmenityId}", request.PropertyTypeId, request.AmenityId);
                return ResultDto<bool>.Failed("حدث خطأ أثناء تخصيص المرفق لنوع العقار");
            }
        }
    }
} 