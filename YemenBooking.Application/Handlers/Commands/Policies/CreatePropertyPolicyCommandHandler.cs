...
{
    /// <summary>
    /// معالج أمر إنشاء سياسة جديدة للعقار
    /// </summary>
    public class CreatePropertyPolicyCommandHandler : IRequestHandler<CreatePropertyPolicyCommand, ResultDto<Guid>>
    {
        ...
        {
            _logger.LogInformation("بدء إنشاء سياسة للعقار: PropertyId={PropertyId}, Type={Type}", request.PropertyId, request.PolicyType);
            ...
            if (_currentUserService.Role != "Admin" && property.OwnerId != _currentUserService.UserId)
                return ResultDto<Guid>.Failed("غير مصرح لك بإنشاء سياسة لهذا العقار");
            ...
            await _auditService.LogBusinessOperationAsync(
                ...
                $"تم إنشاء سياسة جديدة للعقار {request.PropertyId} من النوع {request.PolicyType}",
                created.Id,
                "PropertyPolicy",
