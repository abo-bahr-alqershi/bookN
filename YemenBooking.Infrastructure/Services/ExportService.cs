...
namespace YemenBooking.Infrastructure.Services
{
    ...
    public class ExportService : IExportService
    {
        ...
        {
            _logger.LogInformation("تصدير العقارات إلى Excel: {FileName}", fileName);
            try
            {
            ...
            {
                _logger.LogError(ex, "خطأ أثناء تصدير العقارات إلى Excel");
        ...
        {
            _logger.LogInformation("تصدير العقارات إلى PDF: {FileName}", fileName);
            try
            {
                var data = properties.ToList();
                var path = Path.Combine(_exportFolder, fileName);
                ...
                Document.Create(container =>
                {
                    container.Page(page =>
                    {
                        ...
                        {
                            col.Item().Text("تصدير العقارات").FontSize(20).Bold();
            ...
            {
                _logger.LogError(ex, "خطأ أثناء تصدير العقارات إلى PDF");
                return new ExportResult { IsSuccess = false, ErrorMessage = ex.Message };
            }
        }
        ...
        {
            _logger.LogInformation("تصدير العقارات إلى CSV: {FileName}", fileName);
            try
            {
                var data = properties;
                var path = Path.Combine(_exportFolder, fileName);
            ...
            {
                _logger.LogError(ex, "خطأ أثناء تصدير العقارات إلى CSV");
                return new ExportResult { IsSuccess = false, ErrorMessage = ex.Message };
            }
        }
        ...
        {
            _logger.LogInformation("الحصول على العقارات للتصدير (IncludeInactive: {IncludeInactive})", includeInactive);
        ...
        {
            _logger.LogInformation("تصدير أنواع العقارات إلى Excel: {FileName}", fileName);
            try
            {
            ...
            {
                _logger.LogError(ex, "خطأ أثناء تصدير أنواع العقارات إلى Excel");
                return new ExportResult { IsSuccess = false, ErrorMessage = ex.Message };
            }
        }
        ...
        {
            _logger.LogInformation("تصدير أنواع العقارات إلى PDF: {FileName}", fileName);
            try
            {
                var data = propertyTypes.ToList();
                var path = Path.Combine(_exportFolder, fileName);
                Document.Create(container =>
                {
                    container.Page(page =>
                    {
                        ...
                        page.Content().Column(col =>
                        {
                            col.Item().Text("تصدير أنواع العقارات").FontSize(20).Bold();
                            col.Item().Table(table =>
                            {
            ...
            {
                _logger.LogError(ex, "خطأ أثناء تصدير أنواع العقارات إلى PDF");
                return new ExportResult { IsSuccess = false, ErrorMessage = ex.Message };
            }
        }
        ...
        {
            _logger.LogInformation("تصدير أنواع العقارات إلى CSV: {FileName}", fileName);
            try
            {
                var path = Path.Combine(_exportFolder, fileName);
                await using var writer = new StreamWriter(path);
            ...
            {
                _logger.LogError(ex, "خطأ أثناء تصدير أنواع العقارات إلى CSV");
                return new ExportResult { IsSuccess = false, ErrorMessage = ex.Message };
            }
        }
        ...
        {
            _logger.LogInformation("الحصول على أنواع العقارات للتصدير (IncludeInactive: {IncludeInactive})", includeInactive);
