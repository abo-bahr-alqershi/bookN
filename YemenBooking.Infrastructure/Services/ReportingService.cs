...
namespace YemenBooking.Infrastructure.Services
{
    ...
    public class ReportingService : IReportingService
    {
        ...
        {
            _logger.LogInformation("حساب معدل الإشغال للعقار: {PropertyId}", propertyId);
        ...
        {
            _logger.LogInformation("الحصول على تقرير الإشغال للعقار: {PropertyId}", propertyId);
            var prop = await _propertyRepository.GetPropertyWithUnitsAsync(propertyId, cancellationToken);
            var unitsCount = prop?.Units.Count ?? 0;
        ...
        {
            _logger.LogInformation("الحصول على أداء العقار: {PropertyId}", propertyId);
